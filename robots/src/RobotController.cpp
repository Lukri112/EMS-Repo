...
int main(int argc, char **argv) {
    ...
    while (true) {
		...
		std::cerr << "Your robot type = Mecanum." << std::endl;
                break;

            case 'k':

                robot = new KettenRobot();
		std::cerr << "Your robot type = Ketten." << std::endl;
                break;

            case 'a':

                robot = new AckermannRobot();
		std::cerr << "Your robot type = Ackermann." << std::endl;
                break;

            default:

                std::cerr << "Invalid robot type. Please enter a valid option." << std::endl;
    ...
    while (ros::ok()) {
        ...
        std::cout << "Enter command (wasd = normal, qryc = diagonal, l = left turn, r = right turn, x = Stop): ";
